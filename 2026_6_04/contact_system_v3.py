...
class ContactBook:
    ...
    def load(self):#加载
        try:
            with open("contacts.json","r") as file:
                data=json.load(file)
            for person in data:
                self.contacts.append(Contact(person['name'],person['phone'],person['email']))
        except FileNotFoundError:
            return
    def save(self):#保存  
        data=[]
        for person in self.contacts:
            data.append({"name":person.name,"phone":person.phone,"email":person.email})
        with open("contacts.json","w") as file:
            json.dump(data,file, ensure_ascii=False, indent=4)
    def add(self):
        names={person.name.lower() for person in self.contacts}
        phones={person.phone for person in self.contacts}
        emails={person.email for person in self.contacts}
        while True:
    ...
    def search_by_phone(self):
        while True:
            print("如要退出在任意输入时输入Exit或其首字母")
            phone=input("请输入要查找的联系人电话:").lower().strip()
            if not phone:
                print("暂无相关联系人")
                continue
            if phone in ("exit","e"):
            ...
            findflag=False
            for person in self.contacts:
                if phone in person.phone.lower():
                    print(person.phone)
                    findflag=True
            if findflag:
                print("已显示所有相关联系人,请再次输入以获取详细信息")
...
def main():
    ...
    while True:
        ...
        if choice in ("exit","e"):
            ...
            contactbook.search()
            input("请输入回车返回菜单...")
        elif choice in ("phone","p"):
            contactbook.search_by_phone()
            input("请输入回车返回菜单...")
